...
// CODE HERE
const filteredNumbers = numbers.forEach((element)=>{
...
// CODE HERE
newNumVar = numbers.map((value)=>{
let divideByTwo = value /2
return divideByTwo
})
console.log(newNumVar)
////////// PROBLEM 3 //////////
...
// CODE HERE
NoLetterI = cities.filter((string)=>{
  NewString = string.endsWith('i');
  return NewString
})
console.log(NoLetterI)
...
NewFruits= fruits.forEach((element,index)=>{
  ...
  console.log(`The fruit with index ${index} is an Apple`)
    ...
    console.log(`The fruit at Index ${index} is an Orange`)
  }else{console.log(`The fruit with index ${index} is neither an apple or orange`)
}})
...
// The fruit at Index 1 is an Orange
...
// CODE HERE
totalFoods = foods.reduce((Acc,name,price)=>{
  if(name.tags.includes("rice")){
    Acc += name.price
  }
  return Acc
},0)
